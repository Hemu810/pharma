...
Confirming turns each selected field into a condition with an operator chosen
from how many values were picked (`=` for one, `in` for several, `between`/`≥`/`≤`
for ranges), appended to the query builder with a default join of **AND**.

Each condition's join is a three-state toggle — click the pill in the chain to
cycle **AND → OR → NOT**. "NOT" reads as "AND NOT" in the criteria summary
(i.e. it excludes matches for that condition rather than requiring them).
